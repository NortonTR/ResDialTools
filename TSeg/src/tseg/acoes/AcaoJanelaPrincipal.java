...
public class AcaoJanelaPrincipal {
		...
		try {
			...
			if (scanner.hasNext()) {
				...
				scanner.close();
                                ...
                                //Se a extensao do arquivo aberto nao for xml, e necessario abrir a caixa Salvar Como
                                String extensao = FiltroExtencoes.retornaExtencao(ultimoArquivo);
	...
	private void salvarComo() {
		JFileChooser fc = getFileChooserSalvarComo();
		...
		if (retornoFc == JFileChooser.APPROVE_OPTION) {
			File selectedFile = fc.getSelectedFile();
                        ...
                        if(extensao == null || !extensao.equals("xml"))
                        {
        ...
        private void leUltimoArquivo()
        {
            TratamentoArquivo arquivo = new TratamentoArquivo();
            String textoArquivo = arquivo.leArquivo(Configuracoes.ULTIMO_ARQUIVO);
            Controle.getJanelaPrincipal().getAbaSegmentaca().setTextoTxaSegmentacao(textoArquivo);
        }
        
        public ActionListener itemUTF8() {
		return new ActionListener() {
                    
			@Override
                        public void actionPerformed(ActionEvent evt) {
                                Configuracoes.ENCODING_ATUAL = StandardCharsets.UTF_8;
                                tseg.janelas.JanelaPrincipal.chooseCurrentEncoding();
        ...
        public ActionListener itemIso88591() {
		return new ActionListener() {
                    
			@Override
                        public void actionPerformed(ActionEvent evt) {
                                Configuracoes.ENCODING_ATUAL = StandardCharsets.ISO_8859_1;
                                tseg.janelas.JanelaPrincipal.chooseCurrentEncoding();
        ...
        public ActionListener itemUTF16() {
		return new ActionListener() {
                    
			@Override
                        public void actionPerformed(ActionEvent evt) {
                                Configuracoes.ENCODING_ATUAL = StandardCharsets.UTF_16;
                                tseg.janelas.JanelaPrincipal.chooseCurrentEncoding();
        ...
        public ActionListener itemUsASCII() {
		return new ActionListener() {
                    
			@Override
                        public void actionPerformed(ActionEvent evt) {
                                Configuracoes.ENCODING_ATUAL = StandardCharsets.US_ASCII;
                                tseg.janelas.JanelaPrincipal.chooseCurrentEncoding();
	...
	public static JFileChooser getFileChooserSalvarComo() {
		Configuracoes configuracoes = new Configuracoes();
		JFileChooser fc = new JFileChooser(configuracoes.getDiretorioPadrao());
		...
		fc.setAcceptAllFileFilterUsed(true);
                ...
                FiltroExtencoes filtro = new FiltroExtencoes();
                filtro.acceptsOnlyXML();
		int quantidadeExtencoes = filtro.quantidadeExtencoes();

		for (int i = 0; i <= quantidadeExtencoes; i++) {
			fc.addChoosableFileFilter(new FiltroArquivo(filtro).criaFiltro(i));
		}
		return fc;
	}
